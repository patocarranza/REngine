package org.rosuda.rserve.protocol;

// JRclient library - client interface to Rserve, see http://www.rosuda.org/Rserve/
import java.io.InputStream;
import java.io.OutputStream;
import org.rosuda.rserve.RConnection;

// Copyright (C) 2004 Simon Urbanek
// --- for licensing information see LICENSE file in the original JRclient distribution ---
/**
 * This class encapsulates the QAP1 protocol used by Rserv. it is independent of
 * the underying protocol(s), therefore RTalk can be used over any transport
 * layer
 * <p>
 * The current implementation supports long (0.3+/0102) data format only up to
 * 32-bit and only for incoming packets.
 * <p>
 * @version $Id$
 */
public class RTalk {
  ...
  /**
   * this is a flag saying that the contents is large (>0xfffff0) and hence uses
   * 56-bit length field
   */
  ...
  /**
   * writes bit-wise int to a byte buffer at specified position in Intel-endian
   * form
   *
  ...
  /**
   * writes cmd/resp/type byte + 3/7 bytes len into a byte buffer at specified
   * offset.
   *
   ...
   * @param buf buffer
   * @param o offset
   * @return offset in buf just after the header. Please note that since Rserve
   * 0.3 the header can be either 4 or 8 bytes long, depending on the len
   * parameter.
   */
   ...
   * @return the int value. no bounds checking is done so you need to make sure
  ...
  /**
   * converts bit-wise stored length from a header. "long" format is supported
   * up to 32-bit
   *
   * @param buf buffer
  ...
  /**
   * sends a request with attached prefix and parameters. Both prefix and cont
   * can be <code>null</code>. Effectively <code>request(a,b,null)</code> and
   * <code>request(a,null,b)</code> are equivalent.
   *
   * @param cmd command - a special command of -1 prevents request from sending
   * anything
   * @param prefix - this content is sent *before* cont. It is provided to save
   * memory copy operations where a small header precedes a large data chunk
   * (usually prefix conatins the parameter header and cont contains the actual
   * data).
   * @param cont contents
   * @param offset offset in cont where to start sending (if <0 then 0 is
   * assumed, if >cont.length then no cont is sent)
   * @param len number of bytes in cont to send (it is clipped to the length of
   * cont if necessary)
   * @return returned packet or <code>null</code> if something went wrong
   */
