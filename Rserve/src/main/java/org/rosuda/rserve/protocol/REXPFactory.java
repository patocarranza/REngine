...
public class REXPFactory {
  ...
  /**
   * xpression type: RFactor; this XT is internally generated (ergo is does not
   * come from Rsrv.h) to support RFactor class which is built from XT_ARRAY_INT
   */
  ...
  /**
   * parses byte buffer for binary representation of xpressions - read one
   * xpression slot (descends recursively for aggregated xpressions such as
   * lists, vectors etc.)
   *
   * @param buf buffer containing the binary representation
   * @param o offset in the buffer to start at
   * @return position just behind the parsed xpression. Can be use for
   * successive calls to {@link #parseREXP} if more than one expression is
   * stored in the binary array.
   */
  ...
  /**
   * Calculates the length of the binary representation of the REXP including
   * all headers. This is the amount of memory necessary to store the REXP via
   * {@link #getBinaryRepresentation}.
   * <p>
   * Please note that currently only XT_[ARRAY_]INT, XT_[ARRAY_]DOUBLE and
   * XT_[ARRAY_]STR are supported! All other types will return 4 which is the
   * size of the header.
   *
  ...
  /**
   * Stores the REXP in its binary (ready-to-send) representation including
   * header into a buffer and returns the index of the byte behind the REXP.
   * <p>
   * Please note that currently only XT_[ARRAY_]INT, XT_[ARRAY_]DOUBLE and
   * XT_[ARRAY_]STR are supported! All other types will be stored as SEXP of the
   * length 0 without any contents.
   *
  ...
  /**
   * returns human-readable name of the xpression type as string. Arrays are
   * denoted by a trailing asterisk (*).
   *
