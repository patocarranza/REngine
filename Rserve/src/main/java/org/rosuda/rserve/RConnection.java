...
import org.rosuda.rserve.protocol.REXPFactory;
import org.rosuda.rserve.protocol.RPacket;
import org.rosuda.rserve.protocol.RTalk;
import org.rosuda.rserve.protocol.jcrypt;
...
public class RConnection extends REngine {
  ...
  /**
   * This static variable specifies the character set used to encode string for
   * transfer. Under normal circumstances there should be no reason for changing
   * this variable. The default is UTF-8, which makes sure that 7-bit ASCII
   * characters are sent in a backward-compatible fashion. Currently (Rserve
   * 0.1-7) there is no further conversion on Rserve's side, i.e. the strings
   * are passed to R without re-coding. If necessary the setting should be
   * changed <u>before</u> connecting to the Rserve in case later Rserves will
   * provide a possibility of setting the encoding during the handshake.
   */
  ...
  /**
   * make a new connection to specified host and given port. Make sure you check
   * {@link #isConnected} to ensure the connection was successfully created.
   *
   * @param host host name/IP
   * @param port TCP port
   */
  ...
  /**
   * evaluates the given command, detaches the session (see @link{detach()}) and
   * closes connection while the command is being evaluted (requires Rserve
   * 0.4+). Note that a session cannot be attached again until the commad was
   * successfully processed. Techincally the session is put into listening mode
   * while the command is being evaluated but accept is called only after the
   * command was evaluated. One commonly used techique to monitor detached
   * working sessions is to use second connection to poll the status (e.g.
   * create a temporary file and return the full path before detaching thus
   * allowing new connections to read it).
   *
   * @param cmd command/expression string
   * @return session object that can be use to attach back to the session once
  ...
  /**
   * assign a string value to a symbol in R. The symbol is created if it doesn't
   * exist already.
   *
   * @param sym symbol name. Currently assign uses CMD_setSEXP command of
   * Rserve, i.e. the symbol value is NOT parsed. It is the responsibility of
   * the user to make sure that the symbol name is valid in R (recall the
   * difference between a symbol and an expression!). In fact R will always
   * create the symbol, but it may not be accessible (examples: "bar\nfoo" or
   * "bar$foo").
   * @param ct contents
   */
   ...
   *
   * @param fn file name. should not contain any path delimiters, since Rserve
   * may restrict the access to local working directory.
   * @return input stream to be used for reading. Note that the stream is
   * read-once only, there is no support for seek or rewind.
   */
   ...
   * @param fn file name. should not contain any path delimiters, since Rserve
   * may restrict the access to local working directory.
   * @return output stream to be used for writinging. Note that the stream is
   * write-once only, there is no support for seek or rewind.
   */
  ...
  /**
   * shutdown remote Rserve. Note that some Rserves cannot be shut down from the
   * client side.
   */
   ...
   * All responses send by Rserve are stored in the send buffer before
   * transmitting. This means that any objects you want to get from the Rserve
   * need to fit into that buffer. By default the size of the send buffer is
   * 2MB. If you need to receive larger objects from Rserve, you will need to
   * use this function to enlarge the buffer. In order to save memory, you can
   * also reduce the buffer size once it's not used anymore. Currently the
   * buffer size is only limited by the memory available and/or 1GB (whichever
   * is smaller). Current Rserve implementations won't go below buffer sizes of
   * 32kb though. If the specified buffer size results in 'out of memory' on the
   * server, the corresponding error is sent and the connection is
   * terminated.<br>
   * <i>Note:</i> This command may go away in future versions of Rserve which
   * will use dynamic send buffer allocation.
   *
  ...
  /**
   * set string encoding for this session. It is strongly recommended to make
   * sure the encoding is always set to UTF-8 because that is the only encoding
   * supported by this Java client. It can be done either by uisng the
   * <code>encoding</code> option in the server or by calling
   * setStringEncoding("utf8") at the beginning of a session (but after login).
   *
   * @param enc name of the encoding as defined by Rserve - as of Rserve version
   * 0.5-3 valid values are "utf8", "latin1" and "native" (case-sensitive)
   * @since Rserve 0.5-3
   */
  ...
  /**
   * login using supplied user/pwd. Note that login must be the first command if
   * used
   *
   * @param user username
   * @param pwd password
  ...
  /**
   * detaches the session and closes the connection (requires Rserve 0.4+). The
   * session can be only resumed by calling @link{RSession.attach}
   */
  ...
  /**
   * check connection state. Note that currently this state is not checked
   * on-the-spot, that is if connection went down by an outside event this is
   * not reflected by the flag
   *
   ...
   *
   * @return <code>true</code> is server requires authentication. In such case
   * first command after connecting must be {@link #login}.
   */
  ...
  /**
   * evaluates the given command in the master server process asynchronously
   * (control command). Note that control commands are always asynchronous,
   * i.e., the expression is enqueued for evaluation in the master process and
   * the method returns before the expression is evaluated (in non-parallel
   * builds the client has to close the connection before the expression can be
   * evaluated). There is no way to check for errors and control commands should
   * be sent with utmost care as they can abort the server process. The
   * evaluation has no immediate effect on the client session.
   *
   * @param cmd command/expression string
   * @since Rserve 0.6-0
   */
  ...
  /**
   * sources the given file (the path must be local to the server!) in the
   * master server process asynchronously (control command). See
   * {@link #serverEval()} for details on control commands.
   *
   * @param serverFile path to a file on the server (it is recommended to always
   * use full paths, because the server process has a different working
   * directory than the client child process!).
   * @since Rserve 0.6-0
   */
  ...
  /**
   * attempt to shut down the server process cleanly. Note that there is a
   * fundamental difference between the {@link shutdown()} method and this
   * method: <code>serverShutdown()</code> is a proper control command and thus
   * fully authentication controllable, whereas {@link shutdown()} is a
   * client-side command sent to the client child process and thus relying on
   * the ability of the client to signal the server process which may be
   * disabled. Therefore <code>serverShutdown()</code> is preferred and more
   * reliable for Rserve 0.6-0 and higher.
   *
   * @since Rserve 0.6-0
   */
...
//========= REngine interface API
  ...
  /**
   * fetch the contents of the given reference. The resulting REXP may never be
   * REXPReference.
   *
