 ...
 * Subclasses can implement interfaces to R in many different ways. Clients
 * should always use methods this class instead of its direct subclasses in
 * order to maintian compatibility with any R engine implementation. The
 * canonical way of obtaining a new engine is to call {@link #engineForClass}.
 * All subclasses must implement <code>createEngine()</code> method.
 */
public abstract class REngine {
  ...
  /**
   * this is the designated constructor for REngine classes. It uses reflection
   * to call createEngine method on the given REngine class.
   *
   * @param klass fully qualified class-name of a REngine implementation
   * @return REngine implementation or <code>null</code> if
   * <code>createEngine</code> invokation failed
   */
  ...
  /**
   * This is the extended constructor for REngine classes. It uses reflection to
   * call createEngine method on the given REngine class with some additional
   * control over the engine. Note that not all engines may support the extended
   * version.
   *
   * @param klass fully qualified class-name of a REngine implementation
   * @param args arguments to pass to R for initialization
   * @param callbacks delegate for REngine callbacks or <code>null</code> if
   * callbacks won't be serviced (engine may not support callbacks)
   * @param runREPL if <code>true</code> then REPL will be started (if supported
   * by the engine)
   * @return REngine implementation or <code>null</code> if
   * <code>createEngine</code> invokation failed
   */
   ...
   *
   * @return last created engine or <code>null</code> if no engine was created
   * yet
   */
   ...
   * @param resolve resolve the resulting REXP (<code>true</code>) or just
   * return a reference (<code>false</code>)
   ...
   * @param where environment to evaluate in (use <code>null</code> for the
   * global environemnt and/or if environments are not supported by the engine)
   ...
   * @param env environment to assign to (use <code>null</code> for the global
   * environemnt and/or if environments are not supported by the engine)
   */
   ...
   *
   * @param symbol symbol name
   * @param env environment (use <code>null</code> for the global environemnt
   * and/or if environments are not supported by the engine)
   * @param resolve resolve the resulting REXP or just return a reference
   * @return value
   */
   ...
   * REXPReference. The engine should raise a {@link #REngineException}
   * exception if {@link #supportsReferences()} returns <code>false</code>.
   *
  ...
  /**
   * create a reference by pushing local data to R and returning a reference to
   * the data. If ref is a reference it is returned as-is. The engine should
   * raise a {@link #REngineException} exception if
   * {@link #supportsReferences()} returns <code>false</code>.
   *
  ...
  /**
   * removes reference from the R side. This method is called automatically by
   * the finalizer of <code>REXPReference</code> and should never be called
   * directly.
   *
   ...
   * @param resolve whether to resolve the reference to the environemnt (usually
   * <code>false</code> since the returned environment will be empty)
  ...
  /**
   * convenince method equivalent to
   * <code>eval(parse(text, false), where, resolve);</code>
   *
   * @param text to parse (see {@link #parse})
   * @param where environment to evaluate in (see {@link #eval})
   * @param resolve whether to resolve the resulting reference or not (see
   * {@link #eval})
   * @return result
   */
  ...
  /**
   * convenince method equivalent to
   * <code>eval(parse(cmd, false), null, true);</code>
   *
   ...
   * {@link #eval} and {@link #assign} only support the global environment
  ...
  /**
   * check whether this engine supports REPL (Read-Evaluate-Print-Loop) and
  ...
  /**
   * check whether this engine supports locking ({@link #lock}, {@link #tryLock}
   * and {@link #unlock}).
   *
   * @return <code>true</code> if this engine supports REPL, <code>false/code> otherwise
   */
   ...
   * <code>assign(symbol, new REXPDouble(d), null)</code> (see
   * {@link #assign(String, REXP, REXP)})
   *
  ...
  /**
   * convenience method equivalent to
   * <code>assign(symbol, new REXPInteger(d), null)</code> (see
   * {@link #assign(String, REXP, REXP)})
   *
   * @param symbol symbol name to assign to
   * @param d values to assign
  ...
  /**
   * convenience method equivalent to
   * <code>assign(symbol, new REXPString(d), null)</code> (see
   * {@link #assign(String, REXP, REXP)})
   *
   * @param symbol symbol name to assign to
   * @param d values to assign
  ...
  /**
   * convenience method equivalent to
   * <code>assign(symbol, new REXPRaw(d), null)</code> (see
   * {@link #assign(String, REXP, REXP)})
   *
   * @param symbol symbol name to assign to
   * @param d values to assign
  ...
  /**
   * convenience method equivalent to <code>assign(symbol, value, null)</code>
   * (see {@link #assign(String, REXP, REXP)})
   *
   * @param symbol symbol name to assign to
  ...
  //--- locking API ---
  /**
   * attempts to obtain a lock for this R engine synchronously (without waiting
   * for it).
   * <br>Note: check for {@link #supportsLocking()} before relying on this
   * capability. If not implemented, always returns 0.
   *
   * @return 0 if the lock could not be obtained (R engine is busy) and some
   * other value otherwise -- the returned value must be used in a matching call
   * to {@link #unlock(int)}.
   */
   ...
   * capability. If not implemented, always returns 0.
   *
   * @return value that must be passed to {@link #unlock} in order to release
   * the lock
   */
  ...
  /**
   * releases a lock previously obtained by {@link #lock()} or
   * {@link #tryLock()}.
   * <br>Note: check for {@link #supportsLocking()} before relying on this
   * capability. If not implemented, has no effect.
   *
