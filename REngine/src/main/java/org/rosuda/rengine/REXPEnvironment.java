...
/**
 * REXPEnvironment represents an environment in R. Very much like
 * {@link org.rosuda.REngine.REXPReference} this is a proxy object to the actual
 * object on the R side. It provides methods for accessing the content of the
 * environment. The actual implementation may vary by the back-end used and not
 * all engines support environments. Check
 * {@link org.rosuda.REngine.REngine.supportsEnvironments()} for the given
 * engine. Environments are specific for a given engine, they cannot be passed
 * across engines.
 */
public class REXPEnvironment extends REXP {
  ...
  REngine eng;
  /**
   * transparent handle that can be used by the engine to indentify the
   * environment. It is not used by REngine API itself.
   */
  Object handle;

  /**
   * create a new environemnt reference - this constructor should never be used
   * directly, use {@link REngine.newEnvironment()} instead.
   *
  ...
  /**
   * returns the handle used to identify this environemnt in the engine - for
   * internal use by engine implementations only
   *
   ...
   * @param resolve if <code>false</code> returns a reference to the object, if
   * <code>false</code> the reference is resolved
   * @return value corresponding to the symbol name or possibly
   * <code>null</code> if the value is unbound (the latter is currently
   * engine-specific)
   */
  ...
  /**
   * get a value from this environment - equavalent to
   * <code>get(name, true)</code>.
   *
   * @param name name of the value
   ...
   *
   * @param resolve if <code>true</code> returns the environemnt, otherwise a
   * reference.
  ...
  /**
   * returns the parent environment. This is equivalent to
   * <code>parent(true)</code>.
   *
