...
public class REXPMismatchException extends Exception {
  ...
  /**
   * primary constructor. The exception message will be formed as "attempt to
   * access &lt;REXP-class&gt; as &lt;access-string&gt;"
   *
   * @param sender R object that triggered this exception (cannot be
   * <code>null</code>!)
   * @param access assumed type of the access that was requested. It should be a
   * simple name of the assumed type (e.g. <tt>"vector"</tt>). The type name can
   * be based on R semantics beyond basic types reflected by REXP classes. In
   * cases where certain assertions were not satisfied, the string should be of
   * the form <tt>"type (assertion)"</tt> (e.g. <tt>"data frame (must have
   * dim>0)"</tt>).
   */
   ...
   *
   * @return string describing the access type. See
   * {@link #REXPMismatchException} for details.
   */
