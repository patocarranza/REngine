...
/**
 * representation of a factor variable. In R there is no actual object type
 * called "factor", instead it is coded as an int vector with a list attribute.
 * The parser code of REXP converts such constructs directly into the RFactor
 * objects and defines an own XT_FACTOR type  *
 * @version $Id$
 */
public class RFactor {
   ...
   * @param index_base index of the first level element (1 for R factors, cannot
   * be negtive)
   */
  ...
  /**
   * create a new factor variable by factorizing a given string array. The
   * levels will be created in the orer of appearance.
   *
   * @param c contents
  ...
  /**
   * return <code>true</code> if the factor contains the given level (it is NOT
   * the same as levelIndex==-1!)
   */
  ...
  public int levelIndex(String name) {
    ...
    int i = 0;
    while (i < levels.length) {
      if (levels[i] != null && levels[i].equals(name)) {
        return i + index_base;
      }
      i++;
    }
  ...
  /**
   * return the list of levels (0-based, use {@link #indexBase} correction if
   * you want to access it by level index)
   */
  ...
  /**
   * displayable representation of the factor variable public String toString()
   * { //return
   * "{"+((val==null)?"<null>;":("levels="+val.size()+";"))+((id==null)?"<null>":("cases="+id.size()))+"}";
   * StringBuffer sb=new StringBuffer("{levels=("); if (val==null)
   * sb.append("null"); else for (int i=0;i<val.size();i++) {
   * sb.append((i>0)?",\"":"\""); sb.append((String)val.elementAt(i));
   * sb.append("\""); }; sb.append("),ids=("); if (id==null) sb.append("null");
   * else for (int i=0;i<id.size();i++) { if (i>0) sb.append(",");
   * sb.append((Integer)id.elementAt(i)); }; sb.append(")}"); return
   * sb.toString(); }
   */
}
