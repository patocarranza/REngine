 ...
 * All lists (dotted-pair lists, language lists, expressions and vectors) are
 * regarded as named generic vectors. Note: This implementation has changed
 * radically in Rserve 0.5!
 *
 * This class inofficially implements the Map interface. Unfortunately a
 * conflict in the Java iterface classes Map and List doesn't allow us to
 * implement both officially. Most prominently the Map 'remove' method had to be
 * renamed to removeByKey.
 *
 * @version $Id$
 */
public class RList extends Vector implements List {
   ...
   * constructs an initialized, unnamed list
   *
   * @param contents - a {@link Collection} of {@link REXP}s to use as contents
   * of this list
   */
  ...
  /**
   * constructs an initialized, named list. The length of the contents vector
  ...
  /**
   * constructs an initialized, named list. The size of the contents collection
   * determines the length of the list.
   *
   * @param contents - a {@link Collection} of {@link REXP}s to use as contents
   * of this list
   ...
   *
   * @return <code>true</code> if this list is named, <code>false</code>
   * otherwise
   */
   ...
   *
   * @param i index
   * @return value at the index or <code>null</code> if the index is out of
   * bounds
   */
   ...
   *
   * @param i index
   * @return ket at the index - can be <code>null</code> is the list is unnamed
  ...
  /**
   * set key at the given index. Using this method automatically makes the list
   * a named one even if the key is <code>null</code>. Out of range operations
   * are undefined (currently no-ops)
   *
   * @param i index
  ...
  /**
   * NOTE: THIS IS UNIMPLEMENTED and always returns <code>null</code>! Due to
   * the fact that R lists are not proper maps we canot maintain a set-view of
   * the list
   */
  public Set entrySet() {
    return null;
  ...
  /**
   * Note: sinde RList is not really a Map, the returned set is only an
   * approximation as it cannot reference duplicate or null names that may exist
   * in the list
   */
  public Set keySet() {
    if (names == null) {
